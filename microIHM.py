###############################################################################
#                                                                             #
# microHMI - A light and reliable microcontroller-oriented HMI                #
#                                                                             #
# Copyright (C) Hector Monzon, 2018. Under MIT license                        #
# http://www.opensource.org/licenses/mit-license.php                          #
#                                                                             #
# Version 1.0.3 by Hector Monzon and Eng Jose Barriola                        #
#                                                                             #
# Contact: hectorm0202@gmail.com                                              #
#                                                                             #
# Last edit by HM, 09/02/2018                                                 #
#                                                                             #
###############################################################################
...
class DBScreen(Screen):
    ...
    def export(self, txt, dbs, remlog, *args):
        if not dbs:
            if remlog == '':
                q = session.query(RTU).filter(RTU.RTU_name == self.ids['RTUlst'].sel.text).first()
            else:
                q = session.query(RTU).filter(RTU.RTU_name == remlog).first()
            ...
            if os.path.isfile(os.getcwd() + '\Reports\\' + txt):
                ...
                fl.close()
                ok = Tempop(title = 'Success')
                ok.ids['yo'].text = 'Database exported'
                ok.open()
...
class Savepop(Popup):
    ...
    # If true indicates that a full database is to be imported
    dbaser = BooleanProperty(False)
...
class Chpop(Popup):
    ...
    # Indicates if popup is loading a database
    dbsr = BooleanProperty(False)
    ...
    def load(self, filename,*args):
        ...
        if not self.dbsr:
            ...
            if r not in self.caller.myrtus:
                dbrem = RTU(
                        RTU_name=r.strip()
                        )
                session.add(dbrem)
                ...
                hey = Tempop(title = 'Error')
                hey.ids['yo'].text = 'Name already assigned'
                hey.open()
        else:
            rms = 0
            rs = []
            ...
            for r in rs:
                if r not in self.caller.myrtus:
                    txt = open(filename[0])
                    for num, line in enumerate(txt, 1):
                        if r in line:
                            ds = []
                            ans = []
                            ps = []
                            fillpop = RTUPopup(caller = self.caller, size = (800,600))
                            ln = linecache.getline(filename[0], num + 3)
                            ls = ln.split(':')
                            ds.append(ls[1].strip())
                            ln = linecache.getline(filename[0], num + 6)
                            ls = ln.split(':')
                            ans.append(ls[1].strip())
                            ln = linecache.getline(filename[0], num + 9)
                            ls = ln.split(':')
                            ps.append(ls[1].strip())
                            dbrem = RTU(
                                    RTU_name=r.strip()
                    ...
                    fillpop.comrtu()
                    ok = Tempop(title = 'Success')
                    ok.ids['yo'].text = 'Database loaded'
                    ok.open()
                else:
                    hey = Tempop(title = 'Error')
                    hey.ids['yo'].text = 'Name taken'
                    hey.open()
...
class RTUPopup(Popup):
    ...
    def comrtu(self,*args):
        ...
        if self.nm == '':
            ...
            hey.open()
        elif self.nm in self.caller.myrtus:
            hey = Tempop(title = 'Error')
            hey.ids['yo'].text = 'Name taken'
            hey.open()
        else:
            ...
            self.caller.manager.get_screen('ihmsc').ids['ihm'].ids['remsel'].values.append(self.nm)
...
class Ptspin(Spinner):
    ...
    def setdps(self):
        for f in os.listdir(os.getcwd() + '/Displays'):
            if f.rstrip('.txt') not in self.values:
                self.values.append(f.rstrip('.txt'))
...
class MyLayout(Widget, CompoundSelectionBehavior):
    ...
    def newblinker(self, sbt, nm, xp, yp, *args):
        if sbt == 'a':
            if xp != '0':
                blink1 = Blinker(name = nm, pos = (xp, yp), size = (30, 30), sbt = sbt)
            else:
                blink1 = Blinker(center = self.center, size = (30, 30), sbt = sbt)
            blink1.canvas.get_group('icon')[0].source = os.getcwd() +'/imgs/blinker.png'
        elif sbt == 'b':
            if xp != '0':
                blink1 = Blinker(name = nm, pos = (xp, yp), size = (60, 60), sbt = sbt)
            else:
                blink1 = Blinker(center=self.center, size= (60, 60), sbt = sbt)
            blink1.canvas.get_group('icon')[0].source = os.getcwd() + '/imgs/valve1.png'
        else:
            if xp != '0':
                blink1 = Blinker(name = nm, pos = (xp, yp), size = (50, 50), sbt = sbt)
            else:
                blink1 = Blinker(center = self.center, size = (50, 50), sbt = sbt)
            blink1.canvas.get_group('icon')[0].source = os.getcwd() + '/imgs/pump1.png'
        return blink1

    # Method to add a new outgage
    def newogage(self, sbt, nm, xp, yp, *args):
        if sbt == 'a':
            if xp != '0':
                gage1 = Outgage(name = nm, pos = (xp, yp), size = (25, 100), sbt = sbt)
            else:
                gage1 = Outgage(center = self.center, size = (25, 100), sbt = sbt)
            return gage1
        elif sbt == 'b':
            if xp != '0':
                gage1 = Outgageh(name = nm, pos = (xp, yp), size = (100, 25), sbt = sbt)
            else:
                gage1 = Outgageh(center = self.center, size = (100, 25), sbt = sbt)
            return gage1
        else:
    ...
    def newigage(self, sbt, nm, xp, yp, *args):
        if sbt == 'a':
            if xp != '0':
                gage1 = Ingage(name = nm, pos = (xp, yp), size = (25, 100), sbt = sbt)
            else:
                gage1 = Ingage(center = self.center, size = (25, 100), sbt = sbt)
            gage1.remove_widget(gage1.ids['tank'])
        elif sbt == 'b':
            if xp != '0':
                gage1 = Ingageh(name = nm, pos = (xp, yp), size = (100, 25), sbt = sbt)
            else:
                gage1 = Ingageh(center = self.center, size = (100, 25), sbt = sbt)
        else:
            if xp != '0':
                gage1 = Ingage(name = nm, pos = (xp, yp), size = (25, 100), sbt = sbt)
            else:
    ...
    def portsel(self, *args):
        ...
        if len(qs) == 0:
            ...
            hey.open()
        elif self.ids['dpname'].text == '':
            hey = Tempop(title = 'Error')
            hey.ids['yo'].text = 'Please name display'
            hey.open()
        elif self.ids['remsel'].text == 'Select remote':
            hey = Tempop(title = 'Error')
            hey.ids['yo'].text = 'Please select remote'
            hey.open()
        else:
            if self.ftrun:
    ...
    # Clocks for plotting: Plotting, x-axis labels update and range expansion
    def startgraph(self):
    ...
    def setbg(self, ima, *args):
        with self.canvas.before:
            Rectangle(pos = self.pos, size = self.size, source = ima[0])

    # Makes logs, which are used to reload displays
    def mklog(self):
        self.parent.manager.get_screen('dbsc').export(self.ids['dpname'].text + '_autolog', False, self.ids['remsel'].text)
        fl = open(os.getcwd() + '\Displays\\' + self.ids['dpname'].text + '.txt', 'w+')
        fl.write(self.ids['dpname'].text + '-' + self.myrem + '\r\n')
        for kid in self.toscan:
            fl.write(kid.ptyp + ',' +  kid.sbt + ',' + kid.name + ',' + str(kid.x) + ',' + str(kid.y) + ';' + '\r\n')
        fl.close()

    def loaddp(self, fl):
        dp = open(os.getcwd() + '/Displays/' + fl + '.txt')
        for line in dp:
            if ',' not in line and not line == '\n':
                fln = line.rstrip().split('-')
                ldp = Chpop(caller = self.parent.manager.get_screen('dbsc'))
                ldp.load([os.getcwd() + '/Reports/' + fln[0] + '_autolog.txt'])
                q = session.query(RTU).filter(RTU.RTU_name == fln[1]).first()
                self.ids['dpname'].text = fln[0]
                self.ids['remsel'].text = fln[1]
            else:
                wds = line.rstrip().split(',')
                if wds[0] == 'd':
                    nbl = self.newblinker(wds[1], wds[2], float(wds[3]), float(wds[4].rstrip(';')))
                    self.add_widget(nbl)
                    asv = DBPopup(target_element = nbl, yp = wds[0])
                    asv.ids['rspin'].values.append(q.RTU_name)
                    asv.ids['ptspin1'].text = wds[2]
                    asv.save()
                elif wds[0] == 'a':
                    nig = self.newigage(wds[1], wds[2], float(wds[3]), float(wds[4].rstrip(';')))
                    self.add_widget(nig)
                    asv = DBPopup(target_element = nig, yp = wds[0])
                    asv.ids['rspin'].values.append(q.RTU_name)
                    asv.ids['ptspin1'].text = wds[2]
                    asv.save()
                elif wds[0] == 'p':
                    nog = self.newogage(wds[1], wds[2], float(wds[3]), float(wds[4].rstrip(';')))
                    self.add_widget(nog)
                    asv = DBPopup(target_element = nog, typ = wds[0])
                    asv.ids['rspin'].values.append(q.RTU_name)
                    asv.ids['ptspin1'].text = wds[2]
                    asv.save()
